...
describe('LearnPort web authentication regression coverage', () => {
  ...
  it('provides a dismissible first-use guided demo tour', () => {
    ...
    const shell = readSource('components/AppShell.tsx');
    const styles = readSource('index.css');
    expect(tour).toContain("DEMO_TOUR_STORAGE_PREFIX = 'learnport.demo-tour.role.v2'");
    expect(tour).toContain('ROLE_STEPS');
    expect(tour).toContain('Student learner');
    expect(tour).toContain('Build your learning portfolio');
    expect(tour).toContain('Coordinate training sessions');
    expect(tour).toContain('Work through the marking queue');
    expect(tour).toContain('Review the sampling queue');
    expect(tour).toContain('Inspect the external sample');
    expect(tour).toContain('Approve and manage identities');
    expect(tour).toContain('steps.length');
    expect(tour).toContain('Skip tour');
    expect(tour).toContain('localStorage.setItem');
    expect(tour).toContain('roleKey');
    expect(tour).toContain("user.id === 102 ? 'student'");
    expect(tour).toContain("route: '/dashboard?view=admin'");
