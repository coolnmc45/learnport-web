...
import { demoCriteriaFor, demoMarkingsFor, demoPendingMarking } from '@/lib/demo-data';
...
export function MarkingSuite() {
  ...
  const markingsQuery = trpc.markings.getByAssessor.useQuery({ assessorId: user?.id ?? 0 }, { enabled: user?.role === 'assessor' && !isDemo });
  const criteriaQuery = trpc.learning.getCriteriaByUnit.useQuery({ unitId: selected?.unitId ?? 0 }, { enabled: Boolean(selected) && !isDemo });
  ...
  const submissions = useMemo(() => {
    const pending = (isDemo ? demoPendingMarking() : (pendingQuery.data ?? [])).map((submission: any): SubmissionForMarking => ({ id: submission.id, learnerId: submission.learnerId, unitId: submission.unitId, criterionId: submission.criterionId, fileUrl: submission.fileUrl, learnerName: `Learner #${submission.learnerId}`, unitCode: `Unit #${submission.unitId}`, title: submission.title, submittedDate: dateLabel(submission.submittedAt ?? submission.createdAt), status: 'pending' }));
    const marked = (isDemo ? demoMarkingsFor(user?.id ?? 103) : (markingsQuery.data ?? [])).map((marking: any): SubmissionForMarking => ({ id: marking.submissionId, learnerId: 0, unitId: 0, criterionId: 0, title: `Submission #${marking.submissionId}`, learnerName: 'Learner record', unitCode: 'Unit record', submittedDate: dateLabel(marking.markedAt), status: marking.grade === 'refer' ? 'referred' : 'marked' }));
    const seen = new Set(pending.map((submission) => submission.id));
    return [...pending, ...marked.filter((submission) => !seen.has(submission.id))];
  }, [isDemo, markingsQuery.data, pendingQuery.data, user?.id]);
  ...
  const criteriaRows: CriterionRow[] = useMemo(() => {
    const rows = (isDemo ? demoCriteriaFor(selected?.unitId ?? 0) : (criteriaQuery.data ?? [])).map((criterion: any) => ({ id: criterion.id, code: criterion.code, description: criterion.description }));
  ...
  }, [criteriaQuery.data, isDemo, selected]);
  ...
  const generateFeedback = async () => {
    ...
    if (isDemo) {
      setAiSuggestion(grade === 'refer' ? 'Your submission shows a promising start. Please add a specific workplace example and explain how the relevant guidance shaped your decision.' : grade === 'distinction' ? 'Excellent evidence. Your explanation is detailed, well structured and supported by clear workplace examples.' : 'Good evidence overall. Add one more practical example and connect it directly to the assessment criterion.');
      return;
    }
    try {
  ...
  const submitMarking = async () => {
    ...
    if (isDemo) {
      setNotice(`${selected.title} was saved in the demo workspace as ${grade}. In a connected account this action would persist the marking and notify the learner.`);
      setSelected(null);
      return;
    }
    try {
  ...
  return <div>
    ...
    {isDemo && <div className="notice demo-mode-notice" style={{ marginBottom: 18 }}>Demo workspace: the queue and AI feedback assistant use prepared assessment examples. Saving a mark is simulated locally.</div>}
    {notice && <div className="notice" style={{ marginBottom: 18 }}>{notice}</div>}
    {!isDemo && (pendingQuery.isLoading || markingsQuery.isLoading) && <div className="notice" style={{ marginBottom: 18 }}>Loading persisted assessment records…</div>}
    {!isDemo && (pendingQuery.error || markingsQuery.error) && <div className="notice warning-notice" style={{ marginBottom: 18 }}>The assessment queue could not be loaded. Retry when the database service is reachable.</div>}
    <div className="dashboard-grid"><Metric label="Pending marking" value={String(submissions.filter((item) => item.status === 'pending').length)} detail="Persisted submitted evidence" tone="danger" /><Metric label="Marked records" value={String((isDemo ? demoMarkingsFor(user.id) : (markingsQuery.data ?? [])).length)} detail="Your database marking records" tone="success" /><Metric label="Referred back" value={String(submissions.filter((item) => item.status === 'referred').length)} detail="Feedback action required" tone="warning" /><Metric label="IQA flagged" value={String((isDemo ? demoMarkingsFor(user.id) : (markingsQuery.data ?? [])).filter((marking: any) => marking.flaggedForIqa).length)} detail="Ready for sampling" /></div>
