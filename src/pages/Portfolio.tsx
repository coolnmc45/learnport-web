...
import { DEMO_UNITS, demoPortfolioFor } from '@/lib/demo-data';
...
export function Portfolio() {
  ...
  const portfolio = isDemo ? demoPortfolioFor(user?.id ?? 101) : (portfolioQuery.data ?? { portfolioUnits: [], submissions: [] });
  const submissions = portfolio.submissions ?? [];
  const units = useMemo(() => {
    const unitRows = isDemo ? DEMO_UNITS : (unitsQuery.data ?? []);
  ...
  }, [isDemo, portfolio.portfolioUnits, submissions, unitsQuery.data]);
